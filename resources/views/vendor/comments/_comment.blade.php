    ...
    <div id="comment-{{ $comment->id }}" class="media">
        @else
            <li id="comment-{{ $comment->id }}" class="media">
                @endif
                <img class="mr-3" src="/images/{{ Auth::user()->avatar }}" alt="{{ $comment->commenter->name ?? $comment->guest_name }} Avatar" style="width:32px; height:32px; top:10px; left:10px; border-radius:50%">
                <div class="media-body">
                    <h5 class="mt-0 mb-1">{{ $comment->commenter->firstname ?? $comment->guest_firstname }} {{ $comment->commenter->name ?? $comment->guest_name }} <small class="text-muted">- {{ $comment->created_at->diffForHumans() }}</small></h5>
